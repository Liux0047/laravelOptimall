            ...
            <div class="col-md-4">
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="price-prominent">
                                ...
                                (市场价 <span class="rmb-sign">￥</span><del
                                        id='market_price'>{{ number_format($model->market_price, 2) }}</del>)
                            </span>
                        </td>
                    </tr>
                    <tr>
                ...
                <table class="view-item-table">
                    ...
                    <tr>
                        ...
                        <td>
                            @for ($i=0; $i<$products->count(); $i++)
                                <div class="selection-box color-selection-box @if ($i == 0) selected @endif"
                                     id="color_{{ $products[$i]->product_id }}"
                                     onclick='changeColor({{ $products[$i]->product_id }});'>
                            ...
                            <td>
                                @for ($i=0; $i<$lensTypes->count(); $i++)
                                    <div class="lens-selection-container">
                                        <div class="selection-box lens-selection-box @if ($i == 0) selected @endif"
                                             id="lens_{{ $lensTypes[$i]->lens_type_id }}"
                                             onclick="changeLens({{ $lensTypes[$i]->lens_type_id }}, {{ $lensTypes[$i]->price }}, {{ $model->price }}, {{ $model->market_price }});">
                                            {{ $lensTypes[$i]->title_cn }}
                                            (<strong
                                                    class="rmb-sign">￥{{ number_format($lensTypes[$i]->price, 2) }}</strong>)
                                            <i class='fa fa-check'></i>
                                        </div>
                    ...
                    <tr>
                        <td></td>
                        <td>
                            <button class='btn btn-success view-item-table-btn' type='submit'><i
                                        class='fa fa-credit-card'></i> 立即购买
                            </button>
                            @if (Auth::check())
                                <button class='btn btn-danger view-item-table-btn' type="button"
                                        onclick="addToCart(); return false;" id="add_to_cart_btn">
                                    <i class='fa fa-shopping-cart'></i> 加入购物车
                                </button>
                            @else
                                <a href="{{ action('MemberController@getLogin') }}"
                                   class='btn btn-danger view-item-table-btn'>
                                    <i class='fa fa-shopping-cart'></i> 加入购物车
                                </a>
                            @endif
                        </td>
    ...
    <script type="text/javascript">
        ...
        function postReviewReply(reviewId) {
            ...
            $.ajax({
                ...
                data: {
                    review_id: reviewId, content: content
                },
                datatype: 'json',
                beforeSend: function (request) {
                    return request.setRequestHeader('X-CSRF-Token', $("meta[name='token']").attr('content'));
